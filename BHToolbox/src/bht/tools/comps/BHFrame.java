 ...
 * <h1>WARNING:</h1><!-- <h2>Adding</h2>--> To add components to this, you <b>MUST NOT</b> set the layout to be a
 * {@link java.awt.BorderLayout}. If you do, the components added will not be shown!
 *
 * @author Supuhstar
 * @since Mar 10, 2011
 * @version 1.2.16 - 1.2.16 (2014-10-26) - Kyli Rouge changed {@link CompAction} constant references to {@link Constants}
 * references
 */
@SuppressWarnings(
{
	"StaticNonFinalUsedInInitialization", "UseOfSystemOutOrSystemErr"
})
public class BHFrame extends JFrame implements Saveable, DisplayableNeedle<BHFrame>, BHComponent
{
...
//          winClass = new com.sun.java.swing.plaf.windows.WindowsClassicLookAndFeel();
	...
	static
	{
		...
		try
		{
			...
			settingsSaver.addSaveable(saveableLAFSelection = new SaveableString(LookAndFeelChanger.getLookAndFeel().getClass()
					.getName(), "lafSelection"));
		}
		catch (Throwable t)
		{
			t.printStackTrace();
	 ...
	 * @param force whether to force the look and feel application. See
	 * {@link bht.tools.fx.LookAndFeelChanger#setLookAndFeel(byte, boolean)}
	...
	@SuppressWarnings(
	{
		"UseOfSystemOutOrSystemErr", "OverridableMethodCallInConstructor"
	})
	public BHFrame(CharSequence title, CharSequence version, byte initStyle, byte initLAF, boolean force)
	{
		this(title, (title instanceof StringPP ? (StringPP) title : new StringPP(title)).toAbbreviation(), version, initStyle,
			 initLAF, force);
	}
	...
	public BHFrame(CharSequence title, CharSequence shortTitle, CharSequence version, byte initStyle, byte initLAF,
				   boolean force)
	{
		...
		try
		{
			latestVerURL = new java.net.URL("http://s.supuhstar.operaunite.com/s/content/BH/" + new StringPP(TITLE).toSafeURL()
											+ "/latest.version");
			latestUpdURL = new java.net.URL("http://s.supuhstar.operaunite.com/s/content/BH/" + new StringPP(TITLE).toSafeURL()
											+ "/latest.jar");
		}
		...
		System.out.println("BHFrame:  Initializing settings... (" + bht.tools.util.math.Numbers.groupDigits(System.nanoTime()
																											- last) + ")");
		...
		System.out.println("BHFrame:  Using OS Menu Bar... (" + bht.tools.util.math.Numbers
				.groupDigits(System.nanoTime() - last) + ")");
		last = System.nanoTime();
		BHCompUtilities.setUsesOSMenuBar(true, TITLE);

		System.out.println("BHFrame:  Initializing Components... (" + bht.tools.util.math.Numbers.groupDigits(System.nanoTime()
																											  - last) + ")");
		last = System.nanoTime();
		initComponents();

		System.out.println("BHFrame:  Positioning and fixing UI... (" + bht.tools.util.math.Numbers.groupDigits(System
				.nanoTime() - last) + ")");
//    last = System.nanoTime();
	...
	/**
	 * This method is called from within the constructor to initialize the form. WARNING_MES: Do NOT modify this code. The
	 * content of this method is always regenerated by the Form Editor.
	 */
	@SuppressWarnings(
	{
		"unchecked", "UseOfSystemOutOrSystemErr"
	})
...
//        System.out.println("Component added to actions menu");
	...
	private void defaultVisualStyleMenuItemActionPerformed(java.awt.event.ActionEvent evt)
	{
		try
		{
			...
			if (!YesNoBox.bool("Default color style is prone to bugs. Keep Default color style?", "Default color style applied",
							   YesNoBox.WARNING_MES))
			{
...
//    }
			...
			alertOf(autoCopy ? (a ? "Selected text has been copied to clipboard! " : "Could not copy selected text to clipboard. ")
					: (b ? "Copy successful. " : "Nothing to copy. "),
					(autoCopy ? !a : !b) ? BHMessagePanel.WARNING_MES : BHMessagePanel.INFO_MES);
		}
		catch (Throwable t)
		{
			alertOf(t);
	...
	/**
	 * Returns the {@link javax.swing.JDialog} that containsAll the {@link BHCustColorsEditPanel} that will manage this
	 * program's colors
	 *
	...
	/**
	 * Sets whether the frame should automatically search for selected text to clone when the Copy menu item is selected
	 * (AutoCopier)
	 *
	 ...
	 * {@link alertOf(String message, byte type)} where the {@code type} is {@link bht.tools.Constants#ERROR_MES}, but with
	 * stack printing and conditional message filling
	 *
	...
	public void exit(int status)
	{
		...
		if (exitListeners == null || exitListeners.isEmpty())
		{
			...
			else if (shouldUseAnimations())
			{
				for (java.awt.Window w : getWindows())
					if (w != null && w.isVisible())
						(w instanceof BHComponent ? ((BHComponent) w).getCompAction() : new CompAction())
								.slideClosed(w, true, s);
			}
			else
				System.exit(s);
		}
...
//      mainPanel.add(c, constraints);
	...
	private static void initLAFMenu()
	{
		...
		creation:
		for (final javax.swing.UIManager.LookAndFeelInfo lafi : javax.swing.UIManager.getInstalledLookAndFeels())
			try
			{
...
//              getRootPane().setWindowDecorationStyle(JRootPane.FRAME);
								...
								IllegalAccessException t)
						{
							t.printStackTrace();
							if (YesNoBox.bool(
									"There was an unexpected error when switching Look-And-Feel. Quit program for safety?",
						...
						catch (Throwable t)
						{
							t.printStackTrace();
							if (YesNoBox.bool(
									"There was a particularly unexpected error when switching Look-And-Feel. Quit program for safety?",
									"Error when switching LAF - " + TITLE, YesNoBox.ERROR_MES)) System.exit(0);
						}
				...
				lafRadioButtonMenuItems.getLastItem().setSelected(saveableLAFSelection.getState().equals(lafi.getClassName())
																  || (lafButtonGroup.getSelection() == null
																	  && LookAndFeelChanger.getLookAndFeel(
																	  LookAndFeelChanger.DEFAULT).getName()
																	  .equals(laf.getName())));
				boolean b;
				...
				lafRadioButtonMenuItems.getLastItem().setToolTipText(laf.isSupportedLookAndFeel()
																	 ? "You might not be able to apply the \"" + laf.getName()
																	   + "\" because it is not " + "supported on this system."
																	 : (laf.isNativeLookAndFeel() ? "[NATIVE] - " : "")
																	   + (b ? "Set the Look-And-Feel of the application to \""
																			  + laf.getName() + "\"" : laf.getName()
																									   + " is not supported."));
    ...
    */private java.awt.Container mainPanel;
	...
	/**
	 * Sets whether the application closes when the window is closed. Does not affect the behavior of the Exit menu item.
	 * Setting the default close operation to {@code HIDE_ON_CLOSE} overrides the input of this method.<br/>
	 * <br/>
	...
	public double getMatchStrength(Needle otherSearchable)
	{
		...
		if (otherSearchable.getSearchTitle().toString().equalsIgnoreCase(getTitle()) || new StringPP(otherSearchable
				.getSearchTitle()).containsIgnoreCase(getTitle()) || new StringPP(getTitle()).containsIgnoreCase(otherSearchable
						.getSearchTitle()))
		...
		if (otherSearchable instanceof JFrame)
		{
			for (int i = 0; i < getComponentCount(); i++)
				if (new ArrayPP</*java.awt.Component*/>(((JFrame) otherSearchable).getComponents())
						.contains(getComponents()[i]))
					strength = Math.pow(strength, 0.05);

			for (int i = 0; i < ((JFrame) otherSearchable).getComponentCount(); i++)
				if (new ArrayPP</*java.awt.Component*/>(getComponents()).contains(
								((JFrame) otherSearchable).getComponents()[i]))
					strength = Math.pow(strength, 0.05);
		}

		if (otherSearchable instanceof DisplayableNeedle)
			if (getSearchDisplay() == ((DisplayableNeedle) otherSearchable).getSearchDisplay())
				strength = Math.pow(strength, 0.05);
			else if (getSearchDisplay().equals(((DisplayableNeedle) otherSearchable).getSearchDisplay()))
	...
	@Override
	public BHFrame getSearchDisplay()
	{
		return this;
	}
	...
	@Override
	public BHFrame setSearchDisplay(BHFrame display)
	{
		log.logAction("The frame IS the display", "setSearchDisplay(" + display + ")");
		return this;
	}
	...
	public Saveable loadFromSavedString(CharSequence savedString)
	{
		...
		if (e != -1)
		{
			x = Integer.parseInt(s.substring(b, e));
			b = e + 1;
			e = s.indexOf(SEP, b);
			if (e != -1)
			{
				y = Integer.parseInt(s.substring(b, e));
				b = e + 1;
				e = s.indexOf(SEP, b);
				if (e != -1)
				{
					w = Integer.parseInt(s.substring(b, e));
					b = e + 1;
					e = s.indexOf(SEP, b);
					if (e != -1)
					{
						h = Integer.parseInt(s.substring(b, e));
						setBounds(x, y, w, h);
					}
				}
			}
	...
	public void closeWindow(java.awt.Window w, boolean exit)
	{
		...
		catch (Error er)
		{
			...
			javax.swing.JOptionPane.showMessageDialog(null,
													  "A major error has occurred when closing the window.\nNow exiting for safety reasons",
													  er.getClass().getSimpleName() + " - " + TITLE, YesNoBox.WARNING_MES, Icons
													  .getIcon(Icons.ERROR_32));
...
//          System.out.println("Fetched/This : " + ver + "/" + VERSION);
						...
						try
						{
							new CompAction().morphTo(updateDialog,
													 new java.awt.Rectangle(updateDialog.getX(), updateDialog.getY(),
																			Math.max(updateDialog.getPreferredSize().width,
																					 updateDialog.getWidth()),
																			Math.max(updateDialog.getPreferredSize().height,
																					 updateDialog.getHeight())),
					...
					for (int i1 = ver.indexOf('.'), i2 = V.indexOf('.'),
							...
							p1 = 0, p2 = 0, c;
						 i1 < l1 && i2 < l2;
						 p1 = ++i1, p2 = ++i2, i1 = ver.indexOf('.', i1), i2 = V.indexOf('.', i2), x = i1 == -1 || i2 == -1)
					{
						if (i1 == -1)
							i1 = l1;
						if (i2 == -1)
				...
				{
					updateDialogTextLabel
							.setText("Could not connect to update server. \r\nTry manually downloading the update.");
					updateDialogProgressBar.setVisible(false);
					...
					updateDialogButton.addActionListener(updManualActionListener = new java.awt.event.ActionListener()
					{
						...
						public void actionPerformed(ActionEvent e)
						{
							...
							catch (URISyntaxException | IOException t)
							{
								...
								javax.swing.JOptionPane.showMessageDialog(updateDialog, "Encountered a"
																						+ (c.startsWithVowel() ? "n " : " ")
																								+ c
																						+ " occurred when trying to browse to the URL.\n\n"
																						+ m, c + " - " + TITLE,
																		  Constants.ERROR_MES,
							...
							catch (Throwable t)
							{
								t.printStackTrace();
								StringPP c = new StringPP(t.getClass().getSimpleName()), m = new StringPP(t.getMessage());
								javax.swing.JOptionPane.showMessageDialog(updateDialog, "Unexpectedly encountered a" + (c
																														.startsWithVowel() ? "n " : " ")
																						+ c
																						+ " occurred when trying to browse to the URL.\n\n"
																						+ m, c + " - "
																									 + TITLE,
																		  Constants.ERROR_MES, Icons.getIcon(Icons.ERROR_32));
								closeWindow(updateDialog);
							}
						}
					});
					...
					new CompAction().morphTo(updateDialog,
											 new java.awt.Rectangle(updateDialog.getX(), updateDialog.getY(),
																	Math.max(updateDialog.getPreferredSize().width, updateDialog
																			 .getWidth()),
																	Math.max(updateDialog.getPreferredSize().height,
																			 updateDialog.getHeight())),
											 0, CompAction.DEF_BRAKE, CompAction.DEF_FPS, false, false, null,
				...
				catch (Throwable t)
				{
					t.printStackTrace();
					StringPP c = new StringPP(t.getClass().getSimpleName()), m = new StringPP(t.getMessage());
					javax.swing.JOptionPane.showMessageDialog(updateDialog, "A" + (c.startsWithVowel() ? "n " : " ") + c
																			+ " occurred when trying to check for updates:\n\n"
																			+ m, c + " - " + TITLE, Constants.ERROR_MES, Icons
															  .getIcon(Icons.ERROR_32));
					closeWindow(updateDialog);
				}
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				try
				{
					...
					updateDialogProgressBar.setStringPainted(true);
					updateDialogProgressBar.setIndeterminate(false);
					final java.io.File f = new java.io.File(System.getProperty("user.dir") + java.io.File.separatorChar + TITLE
															+ " " + ver + ".jar");
						...
						public void actionPerformed(ActionEvent e)
						{
							...
							catch (Throwable t)
							{
								...
								javax.swing.JOptionPane.showMessageDialog(updateDialog,
																		  "Could not open the new version due to a" + (c
																													   .startsWithVowel() ? "n " : " ")
																		  + c + ":\n\n" + t.getMessage()
																		  + "\n\nPlease open the JAR manually from your file browser.",
																		  c + " - " + TITLE, Constants.ERROR_MES, Icons.getIcon(
																				  Icons.ERROR_32));
							}
							exit(0);
						}
