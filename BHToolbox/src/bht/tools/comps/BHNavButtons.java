...
/**
 * BHNavButtons, made for BHToolbox, is made by and copyrighted to Blue Husky Programming, ©2012. License is default.<hr/>
    ...
    backButton.addActionListener(new  ActionListener()
    {
      ...
      public void actionPerformed(ActionEvent e)
      {
        NavigationEvent evt = new NavigationEvent(backButton, history.getCurrentIndex(), history.getCurrentIndex() + 1,
                history.getCurrent(), history.getVirtualBack(), NavigationEvent.STATE_GOING_BACKWARD);
    ...
    forwardButton.addActionListener(new  ActionListener()
    {
      ...
      public void actionPerformed(ActionEvent e)
      {
        ...
        NavigationEvent evt = new NavigationEvent(forwardButton, temp = history.getCurrentIndex(), temp + 1,
            history.getCurrent(), history.getVirtualNext(), NavigationEvent.STATE_GOING_FORWARD);
        for (NavigationListener nl : navigationListeners)
          nl.willNavigate(evt);
...
//        histPopupMenu.show(e.getComponent(), e.getX(), e.getY());
  ...
  private void fixHistPopupMenu()
  {
    ...
    for (histPos=0, histLength=history.length(); histPos < histLength; histPos++)
    {
      ...
      if ((b & 0b01) == 0)//!hasHitCurrent
      {
        if (((b |= (histPos >= history.getCurrentIndex() ? 0b01 : 0b00)) & 0b01) != 0)//hasHitCurrent = histPos >= history.getCurrentIndex()
        {
          b |= 0b10;//isCurrent = true
          jmi.setEnabled(false);
        }
      }
      else if ((b & 0b10) != 0)//isCurrent
        b &= ~0b10;//isCurrent = false;
      
      //                  isCurrent                                     hasHitCurrent
      jmi.setToolTipText(((b & 0b10) != 0) ? "You're already here!" : (((b & 0b01) != 0) ? "Go forward to" : "Go back to") +
      ...
      jmi.addActionListener(new ActionListener()
      {
        ...
        {
          int temp;
          NavigationEvent evt = new NavigationEvent(backButton, temp = history.getCurrentIndex(), temp + 1, history.getCurrent(), history.getVirtualBack(), NavigationEvent.STATE_SKIPPING);
          for (NavigationListener nl : navigationListeners)
            nl.willNavigate(evt);
  ...
  /**
   * Removes the given {@link NavigationListener} from the list of listeners that fire whenever a navigation is performed. Only
   * the first encountered instance of the given object is removed. If multiple instances have been added, this method will
