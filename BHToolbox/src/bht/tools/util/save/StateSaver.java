...
/**
 * A class that manages saving and loading of states to any object which extends {@link Saveable}. This class is
 * {@code final}
 * only because extending this fairly basic class leaves too many potentially large and powerful objects untouchable by
 * the
 * child class.
 *
...
public final class StateSaver {
    ...
    /**
     * Creates a new {@code StateSaver} with the given title, using the default save file and and auto-save-load
     * enabled.
     ...
     *
     * @param progTitle    The title of the program for which this {@code StateSaver} is saving
     * @param autoSaveLoad If {@code true}, the program will load the state upon starting and adding a saveable, and
     *                     save upon
     *                     program exit.
     * @param useXML       if {@code true}, the save file will be saved as an {@code .xml} file rather than a
     *                     {@code .ini} file
     */
     ...
     * To make the save file, this first detects whether the user is running Windows. If so, it uses the AppData
     * directory.
     * Else,
     * it uses the default temp directory. Then, it creates a file in that directory plus "\Blue
     * Husky\{@code progTitle}\
    ...
    public StateSaver(CharSequence progTitle, CharSequence saveFileName, boolean autoSaveLoad, boolean useXML) {
        this(
                progTitle,
                SaveConstants.inventSaveFileFor(progTitle, saveFileName + "." + (useXML ? "xml" : "ini")),
                autoSaveLoad
        );
    }
    ...
    /**
     * This is the full constructor. Creates a new {@code StateSaver} with the given {@code progTitle},
     * {@code saveFile}, and
     * {@code autoSaveLoad}
     *
     * @param progTitle    The title of the program for which this {@code StateSaver} is saving
    ...
    public StateSaver(CharSequence progTitle, java.io.File saveFile, boolean autoSaveLoad) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            Logger.getGlobal().log(Level.WARNING, "Could not find save file " + saveFile, ex);
                        }
                        catch (java.io.IOException ex) {
                            Logger.getGlobal().log(Level.WARNING, "Could not save to " + saveFile, ex);
                        }
                    }
                }
                )
        ...
        try {
            ...
            new IOException("Error when trying to add and load the saveable for the auto-save-load feature", t)
                    .printStackTrace();
        }
    ...
    /**
     * Adds the {@link Saveable} {@code s} to the array of things to save and load. If auto-save-load is enabled, the
     * state of
     * this
...
//      Logger.getLogger(getClass().getName()).log(Level.INFO, "Could not create new file: {0}", saveFile.getParent());
        ...
        if (saveFile.getName().endsWith(".xml")) {
            p.storeToXML(new java.io.FileOutputStream(saveFile), "Savestate for " + title + ", made on "
                                                                 + new java.util.Date()
                         .toString(), "UTF-16");
        }
        else {
            p.store(new java.io.FileOutputStream(saveFile), "Savestate for " + title + ", made on "
                                                            + new java.util.Date()
                    .toString());
        }
    ...
    /**
     * Reads the loaded state that correlates to the given {@link Saveable}'s {@link Saveable#getSaveName()} and sets
     * that as
     * the
    ...
    /**
     * Attempts to put the states of all registered {@link Saveable}s. If any one can't be put for any reason, it is
     * skipped and
     * a
