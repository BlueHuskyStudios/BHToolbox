 ...
 * only because extending this fairly basic class leaves too many potentially large and powerful objects untouchable by the
 * child class.
 *
...
public final class StateSaver {
  ...
  //</editor-fold>
	 ...
	 * To make the save file, this first detects whether the user is running Windows. If so, it uses the AppData directory.
	 * Else,
	...
	/**
	 * Adds the {@link Saveable} {@code s} to the array of things to save and load. If auto-save-load is enabled, the state of
	 * this
...
//      Logger.getLogger(getClass().getName()).log(Level.INFO, "Could not create new file: {0}", saveFile.getParent());
		...
		if (saveFile.getName().endsWith(".xml")) {
			p.storeToXML(new java.io.FileOutputStream(saveFile), "Savestate for " + title + ", made on " + new java.util.Date()
						 .toString(), "UTF-16");
		}
		else {
			p.store(new java.io.FileOutputStream(saveFile), "Savestate for " + title + ", made on " + new java.util.Date()
					.toString());
		}
	...
	/**
	 * Reads the loaded state that correlates to the given {@link Saveable}'s {@link Saveable#getSaveName()} and sets that as
	 * the
	...
	/**
	 * Attempts to put the states of all registered {@link Saveable}s. If any one can't be put for any reason, it is skipped and
	 * a
