...
public class Numbers
{
	...
	/**
	 * Used for {@link #max(java.awt.Dimension, java.awt.Dimension, byte)} and {@link #min(java.awt.Dimension,
	 * java.awt.Dimension, byte)} to signify that the returned dimension should have a larger/smaller overall area than the
	 * other.
	 */
	 ...
	 * <LI>{@link #BEHAVIOR_FIT} (or {@code Numbers.BEHAVIOR_HEIGHT | Numbers.BEHAVIOR_WIDTH}) (returns the dimension which is
...
//      l1 = (long)teaseUp(d1); l2 = (long)teaseUp(d2); //my gut tells me to leave this uncommented, but it's causing this method to return zeroes.
  ...
  // </editor-fold>
	 ...
	 * <LI>{@code extractDouble("2.5e1.6", true)} returns the double value of {@code 99.526792638374312692563076271938}</LI>
	 * </UL>
	 *
	 ...
	 * {@code value2} is {@code 45} ({@code 00101101}), where the 5th and 8th bits are on in both values, this will return
	 * {@code false}</LI>
	 * <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is {@code 45} ({@code 00100100}), where
	...
	/**
	 * Returns {@code num} if, and only if, it lies between the given limits. If the limits are equal, returns the limit value.
...
//    }
	...
	/**
	 * Works similarly to the {@code switch} statement. This method returns the given choice out of the given list of choices.
	 ...
	 * @throws ArrayIndexOutOfBoundsException if the given index {@code choice} is less than {@code 0} or greater than or equal
	 ...
	 * <LI>{@code toPrettyDate(43200000006L, Calendar.MILLISECOND)} produces {@code "1 year, 134 days, 6 milliseconds"}</LI>
	 * </UL>
	 *
	...
	/**
	 * Defines that this image should fit within its container upon resizing (if the container is taller or wider than the
	 * image, then the image will stay the same scale so that the complete image is still visible). Equal to {@value}.<hr/>
...
//				continue;
	 ...
	 * If {@code length} is omitted, the substring starting from start until the end of the string will be returned.<BR/>
	...
	public static Range asPHPRange(int size, int start, int length)
	{
		...
		return
			new Range(start, Math.min(start + length, size));
	}
