...
public class Numbers
{
   ...
   * java.awt.Dimension, byte)} to signify that the returned dimension should have a larger/smaller overall perimeter than the
   * other.
   */
  public static final byte BEHAVIOR_PERIMETER = 0;
  /**
   * Used for {@link #max(java.awt.Dimension, java.awt.Dimension, byte)} and {@link #min(java.awt.Dimension,
   * java.awt.Dimension, byte)} to signify that the returned dimension should have a larger/smaller overall area than the other.
   */
  public static final byte BEHAVIOR_AREA = 1;
  /**
   * Used for {@link #max(java.awt.Dimension, java.awt.Dimension, byte)} and {@link #min(java.awt.Dimension,
   * java.awt.Dimension, byte)} to signify that the returned dimension should be wider/thinner than the other.
   */
  public static final byte BEHAVIOR_WIDTH = 2;
  /**
   * Used for {@link #max(java.awt.Dimension, java.awt.Dimension, byte)} and {@link #min(java.awt.Dimension,
   * java.awt.Dimension, byte)} to signify that the returned dimension should be taller/shorter than the other.
   */
  public static final byte BEHAVIOR_HEIGHT = 4;
  //</editor-fold>
   ...
   * @param BEHAVIOR the behavior of the determination algorithm.<br/> <h5>All Possible Uses:</h5> <UL>
   * <LI>{@link #BEHAVIOR_AREA}</LI> <LI>{@link #BEHAVIOR_PERIMETER}</LI> <LI>{@link #BEHAVIOR_HEIGHT}</LI>
   * <LI>{@link #BEHAVIOR_WIDTH}</LI> <LI>{@code Numbers.BEHAVIOR_HEIGHT + Numbers.BEHAVIOR_WIDTH} (returns the dimension which
   * is either taller or wider)</LI> </UL>
   ...
   * <LI>{@link #BEHAVIOR_AREA}</LI> <LI>{@link #BEHAVIOR_PERIMETER}</LI> <LI>{@link #BEHAVIOR_HEIGHT}</LI>
   * <LI>{@link #BEHAVIOR_WIDTH}</LI> <LI>{@code Numbers.BEHAVIOR_HEIGHT + Numbers.BEHAVIOR_WIDTH} (returns the dimension which
   * is either shorter or thinner)</LI> </UL>
   ...
   * {@code "1st"}</LI> <LI>{@code toRank(2)} returns {@code "2nd"}</LI> <LI>{@code toRank(3)} returns {@code "3rd"}</LI>
   * <LI>{@code toRank(4)} returns {@code "4th"}</LI> <LI>{@code toRank(0)} returns {@code "0th"}</LI> <LI>{@code toRank(21)}
  ...
  /**
   * Calculates the ratio between the two given numbers. This uses {@link #getGCDRec(double, double)}. If that fails, it uses
...
//      l1 = (long)teaseUp(d1); l2 = (long)teaseUp(d2); //my gut tells me to leave this uncommented, but it's causing this method to return zeroes.
  ...
  /**
   * Recursively attempts to discover the <b>G</b>reatest <b>C</b>ommon <b>D</b>enominator between the two given numbers
   *
  ...
  /**
   * Iteratively attempts to discover the <b>G</b>reatest <b>C</b>ommon <b>D</b>enominator between the two given numbers
   *
   * @param i1 the first number
   * @param i2 the second number
   ...
   * <LI>{@code d1 == 5}, {@code d2 == 5} <UL><LI>returns {@code true}</LI></UL></LI> <LI>{@code d1 == 5.0001}, {@code d2 == 5}
   * <UL><LI>returns {@code true}</LI></UL></LI> <LI>{@code d1 == 5}, {@code d2 == 5.0001} <UL><LI>returns
   * {@code true}</LI></UL></LI> <LI>{@code d1 == 5.24999}, {@code d2 == 5.25} <UL><LI>returns {@code true}</LI></UL></LI>
  ...
  /**
   * Works like Double.parseDouble, but ignores any extraneous characters. The first radix point ({@code .}) is the only one
   * treated as such.<br/> <h4>Examples:</h4> <UL> <LI>{@code extractDouble("123456.789", false)} returns the double value of
   * {@code 123456.789}</LI> <LI>{@code extractDouble("1qw2e3rty4uiop[5a'6.p7u8&9", false)} returns the double value of
   * {@code 123456.789}</LI> <LI>{@code extractDouble("123,456.7.8.9", false)} returns the double value of
   * {@code 123456.789}</LI> <LI>{@code extractDouble("I have $9,862.39 in the bank.", false)} returns the double value of
   * {@code 9862.39}</LI> <LI>{@code extractDouble("12e6", true)} returns the double value of {@code 12000000}</LI>
   * <LI>{@code extractDouble("2.5e1.6", true)} returns the double value of {@code 99.526792638374312692563076271938}</LI> </UL>
   *
   ...
   * {@code 123456.789}</LI> <LI>{@code extractDouble("123,456.7.8.9")} returns the double value of {@code 123456.789}</LI>
  ...
  public static String groupDigits(double num, byte size, String grouper)
  {
    ...
    for (int i = 0, j = b ? numStr.indexOf(RAD_POI) - 1 : numStr.length() - 1/*, t = b ? numStr.length() : j + 1*/;
         j >= 0; i++, j--)
    {
  ...
  /**
   * Returns the number represented by the given {@code double} {@code num}, formatted to {@code fmt} decimal places: <ol>
   * <LI>{@code X.0, 3} <UL><LI>Returns {@code "X.000"}</LI></UL></LI> <LI>{@code X.00000000Y, 3} <UL><LI>Returns
   * {@code "X.000"}</LI></UL></LI> <LI>{@code X.YZ, 3} <UL><LI>Returns {@code "X.YZ0"}</LI></UL></LI> <LI>{@code X.0YZ, 4}
   * <UL><LI>Returns {@code "X.0YZ0"}</LI></UL></LI> <LI>{@code X.0YZABC, 3} <UL><LI>Returns {@code "X.0YZ"}</LI></UL></LI>
   ...
   * <LI>{@link Double.NEGATIVE_INFINITY} <UL><LI>Returns {@code "-8"}</LI></UL></LI> <LI>{@link Double.NaN} <UL><LI>Returns
   * {@code "?"}</LI></UL></LI> </ol>
   *
   * @param num the number to be formatted
  ...
  /**
   * Returns {@code true} if and only if <b>ALL</b> bits that are on in the second value are also on in the first value
   * <h4>Examples</h4> <UL> <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is
   * {@code 9} ({@code 00001001}), where the 5th and 8th bits are on in both values, and are the ONLY ones on in {@code value2},
   * this will return {@code true}</LI> <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is
   * {@code 45} ({@code 00101101}), where the 5th and 8th bits are on in both values, this will return {@code false}</LI>
   * <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is {@code 45} ({@code 00100100}), where there
   * are no bits on in either of the values, this will return {@code false}</LI> </UL>
   *
   * @param value the value which carries the information bit(s)
   * @param bitsToCheck the value which carries the checking bit(s)
   * @return {@code true} if and only if <b>ALL</b> bits that are on in the second value are also on in the first value
   */
  ...
  /**
   * Returns {@code true} if and only if <b>ANY</b> bits that are on in the second value are also on in the first value
   * <h4>Examples</h4> <UL> <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is
   * {@code 9} ({@code 00001001}), where the 5th and 8th bits are on in both values, and are the ONLY ones on in {@code value2},
   * this will return {@code true}</LI> <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is
   * {@code 45} ({@code 00101101}), where the 5th and 8th bits are on in both values, this will return {@code true}</LI>
   * <LI>when {@code value1} is {@code -55} ({@code 11001001}) and {@code value2} is {@code 45} ({@code 00100100}), where there
   * are no bits on in either of the values, this will return {@code false}</LI> </UL>
   *
   * @param value the value which carries the information bit(s)
   * @param bitsToCheck the value which carries the checking bit(s)
   * @return {@code true} if and only if <b>ANY</b> bits that are on in the second value are also on in the first value
   */
  ...
  /**
   * Returns {@code num} if, and only if, it lies between the given limits. If the limits are equal, returns the limit value. It
   * does not matter which limit is higher.
   *
  ...
  /**
   * Returns this {@code double}'s "toPrettyDate" format: <ol> <LI>{@code X.0} ({@code d} is an integer)<UL><LI>Returns
   * {@code "X"}</LI></UL></LI> <LI>{@code X.00000000Y} ({@code d} is close to an integer)<UL><LI>Returns
   * {@code "X"}</LI></UL></LI> <LI>{@code X.YZ} ({@code d} is definitely not an integer)<UL><LI>Returns
   * {@code "X.YZ"}</LI></UL></LI> <LI>{@link Double.POSITIVE_INFINITY} ({@code d} is infinitely large)<UL><LI>Returns
   * {@code "\u221E"}</LI></UL></LI> <LI>{@link Double.NEGATIVE_INFINITY} ({@code d} is infinitely small)<UL><LI>Returns
   * {@code "-\u221E"}</LI></UL></LI> <LI>{@link Double.NaN} ({@code d} is not a number)<UL><LI>Returns
   * {@code "?"}</LI></UL></LI> </ol>
   *
  ...
  public static String toPrettyString(double d)
  {
    ...
    if (s.length() > 8)
    {
      double e = extractDouble(s.toString().
              substring(0, s.getNumCharsBefore(RAD_POI) + Math.min(8, s.getNumCharsAfter(RAD_POI))));
      return (long) e == e ? Long.toString((long) e) : s.toString();
    }
    if (s.contains('E') && s.contains(RAD_POI))
    {
      long sciNot = (long) extractDouble(s.substring(s.lastIndexOf('E'), s.length()));
    }
    return s.toString();
  }
...
//    }
    ...
    for (byte c = 0; Double.toString(extractDouble(s)).length() >= o.length() && c < 100; c++)
    {
      ...
      {
        is9 = true;
        s = s.substring(0, s.length() - 2) + ((b = Byte.parseByte(Character.toString(s.charAt(s.length() - 2)))) == 9 ? "" : b
                                                                                                                             + 1);
      }
      ...
      s = s.substring(0, s.length() - 1) + Byte.toString((byte) (Byte.parseByte(Character.toString(s.charAt(s.length() - 1)))
                                                                 + 1));
  ...
  /**
   * Works similarly to the {@code switch} statement. This method returns the given choice out of the given list of choices. For
   * instance, {@code choose(2, "Hello", "World!", "Foo", "Bar")} would return {@code "Foo"}
   *
   ...
   * @throws ArrayIndexOutOfBoundsException if the given index {@code choice} is less than {@code 0} or greater than or equal to
   * the number of given choices
   */
  ...
  /**
   * Returns a "pretty" version of the given date.<br/> <h4>Examples</h4> <UL> <LI>{@code toPrettyDate(1000, Calendar.SECOND)}
   ...
   * <LI>{@code toPrettyDate(1000000000, Calendar.SECOND)} produces {@code "11 days, 13 hours, 46 minutes, 40 seconds"}</LI>
   ...
   * <LI>{@code toPrettyDate(43200000006L, Calendar.MILLISECOND)} produces {@code "1 year, 134 days, 6 milliseconds"}</LI> </UL>
   *
  ...
  public static CharSequence toPrettyDate(double timeInMillis, int limit)
  {
    ...
    boolean useMs = (limit == Calendar.MILLISECOND),
            ...
            useD = (useH || limit == Calendar.DAY_OF_YEAR || limit == Calendar.DAY_OF_MONTH || limit == Calendar.DAY_OF_WEEK
   ...
   * then the image will stay the same scale so that the complete image is still visible). Equal to {@value}.<hr/> Behaviors can
   * be combined with the {@code |} (OR) operator
   */
  public static final byte DIM_BEHAVIOR_SCALE_FIT = 0b0100;
  /**
   * Defines that this image should fit within its container upon resizing (if the container is taller or wider than the image,
   * then the image will be scaled and trimmed so that the entire container is filled). Equal to {@value}.<hr/> Behaviors can be
   * combined with the {@code |} (OR) operator
   */
   ...
   *
   * @param objDim the dimensions of the object to be scaled, as represented by a {@link java.awt.Dimension} object
...
//        System.out.println("  Image is wider");
  ...
  /**
   * Returns the bits stored within the given {@code long} from {@code fromIndex} (inclusive) to {@code toIndex} (inclusive)
   *
   * @param value
   * @param fromIndex
   * @param toIndex
  ...
  public static long getBits(long value, byte fromIndex, byte toIndex)
  {
     ...
     * (value & ((Math.pow(2, Math.abs(6 - 2) + 1) - 1) << d)) >> d == 0000 0000 0010 0100 >> 2 == 0000 0000 0000 1001 == 01001 == 9
     * return 9
     */
    /*
     * getBits(10662, 3, 8): (should return all bits from index 8 to 3, which are 110100 in binary and 100 in decimal)
     * value == 10662 == 0010 1001 1010 0110
     * fromIndex == 3
     * toIndex == 8
     * d == Math.min(3, 8) == 3
     ...
     * (value & ((Math.pow(2, Math.abs(3 - 8) + 1) - 1) << d)) >> d == 0000 0000 1010 0000 >> 3 == 0000 0000 0001 0100 == 10100 == 20
     * return 20
     */
  }
  /**
  ...
  /**
   * Creates a human-readable string that represents the given number of bytes <H4>Examples:</H4> <UL> <LI>{@code toBytes(5)}
   * returns "{@code 5 B}"</LI> <LI>{@code toBytes(1024)} returns "{@code 1 KB}"</LI> <LI>{@code toBytes(2048)} returns
   * "{@code 2 KB}"</LI> <LI>{@code toBytes(18236)} returns "{@code 17.80859375 KB}"</LI> <LI>{@code toBytes(5      * 1024 * 1024)}
   ...
   * <LI>{@code toBytes(8000.0 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024)} returns "{@code 8000 YB}"</LI> </UL> Anything
   * higher than 1024<sup>8</sup> will be shown in terms of Yottabytes
   *
   * @param bytes the number of bytes to be interpreted. This is only in {@code double} format so that very large numbers
    ...
    if (bytes >= 0x400)//1024 (Kilobyte)
      if (bytes >= 0x100000)//1024^2 (Megabyte)
        if (bytes >= 0x40000000)//1024^3 (Gigabyte)
          ...
          else return toPrettyString(bytes / 1073741824.0) + "GiB";//Gigabytes
        else return toPrettyString(bytes / 1048576.0) + "MiB";//Megabytes
      else return toPrettyString(bytes / 1024.0) + "KiB";//Kilobytes
    return toPrettyString(bytes) + "B";//Bytes    //Added toPrettyString 2012/10/21 (2.1.14) for PiStory
  }

  /**
   * Return the maximum value of the given list of doubles as quickly as possible.<BR/> <STRONG>Special Cases:</STRONG> <UL>
  ...
  /**
   * Return the number of values in the given list of doubles that are repeated. If there are no repeats, an empty array is
  ...
  /**
   * Returns the <strong>location</strong> of the last occurrence of an even integer in the given list, or -1 if none is
   * encountered
   *
  ...
  /**
   * Defines the behavior of the {@link #round(byte, double)} method so that it returns the given number rounded up to an
   * integer
   */
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println("\"11 days, 13 hours, 46 minutes, 40 seconds\": \"" + toPrettyDate(1000000000, Calendar.SECOND) + "\"");//
    ...
    double x, y, e, i, r;
    System.out.println("GCD of " + (x = 8) + " and " + (y = 4) + " is " + (e = 4) + ": " + (((i = getGCDItr(x, y)) - e) + ((r =
                                                                                                                            getGCDRec(
                                                                                                                            x, y))
                                                                                                                           - e)
                                                                                            == 0 ? "confirmed!"
    ...
    System.out.println("GCD of " + (x = 875) + " and " + (y = 125) + " is " + (e = 125) + ": " + (((i = getGCDItr(x, y)) - e)
                                                                                                  + ((r = getGCDRec(x, y)) - e)
                                                                                                  == 0 ? "confirmed!"
                                                                                                  : "discrepency! (itr = " + i
                                                                                                    + " and, rec = " + r + ")"));

    System.out.println("GCD of " + (x = 300) + " and " + (y = 588) + " is " + (e = 12) + ": " + (((i = getGCDItr(x, y)) - e)
                                                                                                 + ((r = getGCDRec(x, y)) - e)
                                                                                                 == 0 ? "confirmed!"
                                                                                                 : "discrepency! (itr = " + i
                                                                                                   + " and, rec = " + r + ")"));

    System.out.println("GCD of " + (x = 123) + " and " + (y = 456) + " is " + (e = 3) + ": " + (((i = getGCDItr(x, y)) - e)
                                                                                                + ((r = getGCDRec(x, y)) - e)
                                                                                                == 0 ? "confirmed!"
                                                                                                : "discrepency! (itr = " + i
                                                                                                  + " and, rec = " + r + ")"));

    System.out.println("GCD of " + (x = 3141592654L) + " and " + (y = 2718281828L) + " is " + (e = 2) + ": " + (((i = getGCDItr(
                                                                                                                  x, y)) - e)
                                                                                                                + ((r =
                                                                                                                    getGCDRec(x,
                                                                                                                              y))
    ...
    System.out.println("GCD of " + (x = 123456789L) + " and " + (y = 987654321L) + " is " + (e = 9) + ": " + (((i = getGCDItr(x,
                                                                                                                              y))
                                                                                                               - e) + ((r =
                                                                                                                        getGCDRec(
                                                                                                                        x, y))
