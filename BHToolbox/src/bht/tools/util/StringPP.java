...
/**
 * <strong>Copyright Blue Husky Programming, © 2011</strong><br/> {@code StringPP} is to {@code String} as
 * {@code ArrayPP} is to
 * arrays. This class was made to give enhanced usability to character strings, with several convenience methods and
 * several
 ...
 * <pre>
 *		- 1.5.1 (2015-08-30)
 *			+ Kyli added {@link #containsIgnoreCase(char)}
 *		- 1.5.0 (2015-02-22)
 *			+ Kyli added {@link #explode(CharSequence)}, made {@link #getWords()} use it
 *			+ Kyli added {@link #getStringToSave()}
 *			! Kyli fixed bug where {@link #StringPP(Object)} would only convert arrays and iterables
 *			+ Kyli added documentation
 *			. Kyli performed general cleanup
...
public class StringPP implements java.io.Serializable, Comparable<CharSequence>, CharSequence, /* Iterable<StringPP>, */
                                 Iterable<Character>, ToPrimitives, Saveable, Cloneable {
    private static final long serialVersionUID = 0x001_005_0000L;

    /**
     * Checks if the {@code String++} starts with a vowel. This method defines a vowel as the upper or lower case
     * versions of
     * the following:<br/>
...
// </editor-fold>
    ...
    /**
     * If the given {@link CharSequence} is already a {@link StringPP}, then it is returned. Else, a new
     * {@link StringPP} is
     * created out of it
     *
...
//  }
    ...
    /**
     * Creates a new {@code String++} with the same characters as those in the provided {@code ArrayPP} of
     * {@code Character}s
     *
    ...
    /**
     * Creates a new {@code String++} with the characters provided. This is also the catch-all constructor that supports
     * the
     ...
     * <LI>If {@code o} is an {@link Iterable} or an array, then this {@code String++} is comprised of all items in
     * {@code o}
    ...
    public StringPP(Object o) {
        ...
        } else if (o instanceof CharSequence) // Added 2015-02-22 for Too Many Fonts
        {
    ...
    /**
     * Appends the characters in the given {@code CharSequence} to the end of this {@code String++}, followed by a
     * new-line
     ...
     *
     * @version 1.0.1
     * - 1.0.1 (2015-02-22) - Kyli condensed to 1 line
     */
    @Override
    @SuppressWarnings({"CloneDeclaresCloneNotSupported", "CloneDoesntCallSuperClone"})
    public StringPP clone() {
        return new StringPP(chars);
    }

    /**
     * Compares this {@code String++} with another {@code CharSequence}. They are compared, character-for-character. If
     * any one
     * character is different, the difference between the corresponding characters is returned. Else, the difference
     * between the
    ...
    /**
     * Returns {@code true} if and only if this {@code StringPP} contains the given sequence of characters in the order
     * in which
     * they are given
     *
     * @param cs The sequence of characters to be searched for within this {@code StringPP}
     *
     * @return {@code true} if and only if this {@code StringPP} contains the given sequence of characters in the order
     *         in which
     *         they are given
     */
    ...
    /**
     * Returns {@code true} if and only if this {@code StringPP} contains the given character, ignoring what case the
     * characters are
     *
     * @param c The character to be searched for within this {@code StringPP}
     *
     * @return {@code true} if and only if this {@code StringPP} contains the given character, whether or not they are
     *         upper or lower case
     *
     * @since 2015-08-30; 1.5.1
     */
    ...
    /**
     * Detects whether this ends with the given character sequence, ignoring case.
     *
     * @param cs the sequence to detect
     *
     * @return {@code true} iff this ends with the given character sequence,r egardless of case
     */
    public boolean endsWithIgnoreCase(CharSequence cs) {
        return cs != null
                       && length() >= cs.length()
                       && substring(length() - cs.length(), length()).equalsIgnoreCase(cs);
    }
     ...
     *
     * @param o the {@code Object} whose {@code toString()} method's return value will be compared to this
     *          {@code String++}
     *
     * @return {@code true} if and only if: <ol> <li>The strings are of the same length</li> <li>The strings contain the
     *         same
     *         characters</li> </ol>
     */
    @Override
     ...
     */
    // they're compatible.
    @SuppressWarnings("IncompatibleEquals") // they're compatible.
    public boolean equalsIgnoreCase(Object s) {
        return toLowerCase().equals(String.valueOf(s).toLowerCase());
    }

    /**
     * Returns an array of strings, each of which is a substring of this formed by splitting it on boundaries formed by
     * the
     * given delimiter.
     *
     * <p>
     * f {@code delimiter} is the empty string ({@code ""}), this will return {@code null}. If {@code delimiter}
     * contains a
     * value that is not contained in this, an array containing solely this will be returned.</p>
     *
     * <p>
     * Behavior should be identical to <a href="http://php.net/manual/en/function.explode.php">PHP's
     * {@code explode(this, delimiter, INT_MAX}}</a></p>
     *
     * @param delimiter the boundary string
     *
     * @return an array of strings, each of which is a substring of this formed by splitting it on boundaries formed by
     *         the
     *         given delimiter.
     *
     * @since 2015-02-22 (1.5.0)
     * @version 1.0.0
     * - 1.0.0 (2015-02-22) - Kyli made this for Too Many Fonts, inspired by PHP.
     */
    @SuppressWarnings({"UnnecessaryLabelOnBreakStatement", "UnnecessaryLabelOnContinueStatement"})
    public ArrayPP<StringPP> explode(CharSequence delimiter) {
        ArrayPP<StringPP> ret = new ArrayPP<>();
        char[] myChars = toCharArray(), delChars = delimiter.toString().toCharArray();
        StringBuilder sb = new StringBuilder();
        char c;

scan:   for (int n = 0; n < myChars.length; n++) // iterate through each character individually
        {
            c = myChars[n]; // save the current character as c
            boolean isDelimiter = false; // this will determine if we are to add the current string or skip it
            if (c == delChars[0]) // if c is the first character in the delimiter
            {
                isDelimiter = true; // assume this is the delimiter
delScan:        for ( // for each character in the delimiter
                        int m = 0, // m will count the delimiter characters
                        l = min(delChars.length, myChars.length - n); // l is length of the delimiter, or of this minus our progress
                        m < l; m++) {
                    if (delChars[m] != myChars[m + n]) // if any one character does not match up
                    {
                        isDelimiter = false; // this is not the delimiter
                        n = m + n; // set n to after the delimiter
                        break delScan;
                    }
                }
            }
            if (isDelimiter) // if we've got the delimiter
            {
                ret.add(valueOf(sb)); // add the current string
                sb = new StringBuilder(); // reset the builder
                continue scan; // start from the next character
            }

            sb.append(c); // if this is not a delimiter, add the character
        }
        ret.add(valueOf(sb)); // add the final string
        return ret;
    }

    /**
     * Returns the number of characters in this {@code StringPP} after {@code c}. If this {@code StringPP} does not
     * contain
    ...
    /**
     * Returns the number of characters in this {@code StringPP} before {@code c}. If this {@code StringPP} does not
     * contain
     * {@code c}, then {@code -1} is returned.
     *
    ...
    /**
     * Progressively searches for whitespace characters in the StringPP and interprets the surrounding characters as
     * words. The
     * word found and the provided index is returned. If the index is greater than the number of words in the StringPP,
     * then
    ...
    public StringPP getWord(int index) // throws IndexOutOfBoundsException - Removed march 6, 2012 (1.2.7) for Marian
    {
        ...
        /* vitrified 2015-02-22 (made obsolete by #explode(CharSequence))
         * boolean cap = true;
         * for (int i = 0, words = 0;
         * i < length();
         * i++)
         ...
         * cap = false;
         * }
         * return null;//throw new IndexOutOfBoundsException("Index is greater than the number of words in the StringPP
         * (" +
         * index + ")");
         */
    }
     ...
     *
     * @return the index of the first occurrence of the given character AFTER the index {@code offset}, or {@code -1} if
     *         the
    ...
    /**
     * Checks if a certain character in the {@code String++} is a vowel (a vowel is defined by the
     * {@code isVowel(char c)}
     * method.
     *
    ...
    @Override
    public int length() // throws IndexOutOfBoundsException - Removed march 6, 2012 (1.2.7) for Marian
    {
        return chars.length();
    }
    ...
    /**
     * Finds and returns the index of the {@code n}th occurrence of the given {@code CharSequence}. the exact occurrence
     * number
     ...
     * @param removeAll if {@code true}, all instances of {@code cs} will be removed. Else, only the first instance will
     *                  be
     *
     * @return
     */
    public StringPP remove(CharSequence cs, boolean removeAll)// throws IndexOutOfBoundsException - Removed march 6, 2012 (1.2.7) for Marian
    {
     ...
     * @param removeAll if {@code true}, all instances of {@code c} will be removed. Else, only the first instance will
     *                  be
     *
     * @return the resulting {@code String++}
     */
     ...
     *
     * @param cs        the character(s) which will be searched for and removed, if found
     * @param removeAll if {@code true}, all instances of each character in {@code cs} will be removed. Else, only the
     *                  first
     ...
     * @return this
     */
    public StringPP removeWord(int index) // throws IndexOutOfBoundsException - Removed march 6, 2012 (1.2.7) for Marian
    {
     ...
     * @param replaceAll if {@code true}, replaces each and every instance of {@code a} in this {@code String++} with
     *                   {@code b},
    ...
    /**
     * Returns a {@link CharSequence} consisting of all characters from {@code start} (inclusive) and {@code end}
     * (exclusive)
     *
    ...
    public StringPP toAbbreviation() {
        String ret = /* !isEmpty()
                 * &&
                 * !Character.isWhitespace(charAt(0)) ? Character.toString(charAt(0)) : */ "";
    ...
    /**
     * Returns a <i>clone</i> of this string with all letters in the {@code String++} changed to lower-case.
     * <h3>Example:</h3>
...
//    System.out.println("After toLowerCase: " + s + " from " + this);
    ...
    /**
     * Returns a clone of this string, wherein all characters have been changed to ones easily interpretable by a
     * browser. Do
     * not input the entire URL. For instance, if you only want the last half of the URL to be interpreted, that must be
     * all you
     ...
     * {@code java.io.IOException} when you try to browse to it using methods
     * like{@code java.awt.Desktop.browse(URI uri)}.
     ...
     *
     * @return a clone of this string, wherein all characters have been changed to ones easily interpretable by a
     *         browser
     */
    ...
    /**
     * Changes all letters in the {@code String++} to sentence-case letters. <h3>Example:</h3> "the Quick Brown fox
     * jumps over
    ...
    /**
     * Returns a {@code String} containing the characters in this sequence in the same order as this sequence. The
     * length of the
    ...
    /**
     * Returns this string, as an array of strings, where each item in the array is a single character from this string,
     * in
     * order.
     *
     * @return all characters in this string, as an array of strings.
     */
    public ArrayPP<StringPP> toStringArray() {
        ArrayPP<StringPP> ret = new ArrayPP<>();
    ...
    /**
     * Changes all letters in the {@code String++} to title-case letters. <h3>Example:</h3> "The quick brown fox jumps
     * over the
    ...
    /**
     * Changes all letters in the {@code String++} to toggle-case letters. <h3>Example:</h3> "The quick brown fox jumps
     * over the
     * lazy dog." becomes "ThE qUiCk BrOwN FoX jUmPs OvEr ThE lAzY dOg."
     *
     * @return the resulting {@code String++}
    ...
    /**
     * Returns a <i>clone</i> of this string with all letters in the {@code String++} changed to upper-case.
     * <h3>Example:</h3>
     * "the Quick Brown fox jumps over the lazy dog." becomes "the quick brown fox jumps over the lazy dog."
     *
     * @return the resulting {@code String++}
