...
/**
 *
 * Searcher, made for BHToolbox, is made by and copyrighted to Blue Husky Programming, ©2012. License is default.<hr/>
...
//    return java.util.Arrays.binarySearch(a.toArray(), target);
  ...
  {
    String one = first instanceof Searchable ? ((Searchable)first).getSearchTitle() + ((Searchable)first).getKeywords().toString()
            : first.toString(),
           two = second instanceof Searchable ? ((Searchable)second).getSearchTitle() + ((Searchable)second).getKeywords().toString()
            : second.toString();
   ...
   * @return an array of items matching the <tt>Searchable</tt> version of <tt>target</tt>, sorted from best to worst match.
   */
  public ArrayPP<T> smartSearch(ArrayPP<T> a, T target, double cutoff)//Efficiencies added Mar 11, 2012 (1.0.2) for marian
  {
    ...
    Searchable sTarg = target instanceof Searchable ? (Searchable)target : makeSearchable(target);
    
    T t;
    for(int i=0; i < a.length(); i++)
      order.add(((t = a.get(i)) instanceof Searchable ? (Searchable)t : makeSearchable(t)).getMatchStrength(sTarg));
  ...
  /**
   * Transforms any object into a {@code Searchable}. Special cases:
   * <ul>
   * <li><b>{@code Searchable}</b>: Returns the provided object</li>
   ...
   * </ul>
   * @param anObject The object to transform into a {@code Searchable}
   * @return the resulting {@code Searchable}
   * @version 1.1.0
   */
  public static Searchable makeSearchable(Object anObject)
  {
    if (anObject instanceof Searchable)
      return (Searchable)anObject;
    
    Searchable s = new DefSearchable();
